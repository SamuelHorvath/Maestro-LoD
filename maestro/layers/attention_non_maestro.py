...
# TEST OF IMPLEMENTATION BELOW
# from maestro.layers.transformer_non_maestro import MultiheadAttention
# from torch import nn
# import torch
# embed_dim = 256
# num_heads = 8
# key = torch.rand((10, 32, embed_dim))
# query = torch.rand((10, 32, embed_dim))
# value = torch.rand((10, 32, embed_dim))
# key_padding_mask = torch.rand((32, 10)) > 0.5
# attn_mask = torch.rand((10, 10)) > 0.5
# attention = MultiheadAttention(
#   embed_dim, num_heads, add_bias_kv=True, add_zero_attn=True)
# output, attn_weights = attention(
#   query, key, value, attn_mask=attn_mask, key_padding_mask=key_padding_mask)
# print(f"output.shape {output.shape}")
# print(f"attn_weights.shape {attn_weights.shape}")
# att_nn = nn.MultiheadAttention(
#   embed_dim, num_heads, add_bias_kv=True, add_zero_attn=True)
# output_nn, attn_weights_nn = att_nn(
#   query, key, value, attn_mask=attn_mask, key_padding_mask=key_padding_mask)
# print(f"output_nn.shape {output_nn.shape}")
# print(f"attn_weights_nn.shape {attn_weights_nn.shape}")

# attention.q_proj.weight.data = att_nn.in_proj_weight.data[:embed_dim, :]
# attention.k_proj.weight.data = att_nn.in_proj_weight.data[
#   embed_dim:2*embed_dim, :]
# attention.v_proj.weight.data = att_nn.in_proj_weight.data[2*embed_dim:, :]
# attention.out_proj.weight.data = att_nn.out_proj.weight.data

# attention.q_proj.bias.data = att_nn.in_proj_bias.data[:embed_dim]
# attention.k_proj.bias.data = att_nn.in_proj_bias.data[embed_dim:2*embed_dim]
# attention.v_proj.bias.data = att_nn.in_proj_bias.data[2*embed_dim:]
# attention.out_proj.bias.data = att_nn.out_proj.bias.data

# attention.bias_k.data = att_nn.bias_k.data
# attention.bias_v.data = att_nn.bias_v.data

# output, attn_weights = attention(
# query, key, value, attn_mask=attn_mask, key_padding_mask=key_padding_mask)
# print(f"Norm of difference between output and output_nn:"
#       f" {(output - output_nn).norm()}")
# print(f"Norm of difference between attn_weights and attn_weights_nn:"
#       f" {(attn_weights - attn_weights_nn).norm()}")
